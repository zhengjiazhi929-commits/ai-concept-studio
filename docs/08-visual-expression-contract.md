...
- 注册图标不得充当卡片徽章、贴边附件、标题前缀或结果卡注释，也不得依附卡片边框、编号或正文定位。`graph-node` 必须成为连接线真实端点，并取代同一语义的文字节点；同一次“图标 + 标题 + 说明”测量必须产生包含 padding 的 DOM `renderGeometry` 与不含透明 padding 的 `connectorGeometry`，二者同心且后者嵌套于前者。连线只命中可见内容范围，不能停在透明留白外侧，也不能继续使用远大于可见内容的整块布局锚点。`owned-callout` 必须带 owner、归属线和最大 48px 距离；同一 `semanticObjectId` 禁止出现两份主表现；
...
### 3.1 长片解释方式不能退化成同一种卡片模板

`visualMode` 只说明画面使用卡片、开放图解还是二者组合，不足以说明图形怎样帮助理解。需要图解叙事的长片场景必须声明 `narrativeTreatment`、非空 `narrativeTreatmentRationale` 和 `treatmentEvidence`：前两项选择可复用教学方式并解释它怎样减少记忆、比较或因果理解成本；证据必须引用场内真实可见对象、带 `semanticType` 的真实关系和该方式对应的语义角色拓扑。纯标题、转场或不需要额外图解的段落可以显式使用 `not-applicable`，并排除多样性统计；不得为了凑类型添加图案，也不能只换 metadata 标签伪造多样性。

允许的解释方式包括：`package-anatomy`（组成与职责）、`runtime-resource-flow`（运行时按需进入与退出）、`governance-evidence-trail`（证据、审核、反馈和回退）、`state-transformation`、`decision-path`、`comparison-field`、`relationship-map`。一个 8 场以上的 AI 技术长片至少使用 3 种；同一种方式最多连续 2 场，任意连续 4 场不能全部同构。该规则约束表达逻辑，不要求每场都画图标，也不允许为了凑类型添加无语义图案。

形状语法需要在首次混合出现时用一次短暂提示教给观众，之后不常驻：完整闭合边框表示需要整体读取的完整对象或边界；开放节点表示过程或关系锚点；虚线只表示明确的范围、排除项、待验证或风险边界。每个可见虚线实例都必须登记 `semanticBoundaries` 的 `id/meaning/memberIds/rationale`，成员只能是开放图解，meaning 只能是 `scope`、`exclusion`、`pending-validation` 或 `risk-boundary`；普通泳道、装饰分组和信息卡不得画成虚线。选择仍由 `semanticGroupId`、`semanticRole`、层级和真实关系拓扑共同决定，不是整屏“全卡片”或“全开放”的绝对规则。

字幕必须与图解构建协调。图建立前可以显示完整句；节点和关系正在建立时改为不超过 16 个字符、与当前阶段同步的语义短提示，并降为 supporting 视觉权重。最终稳定保持若仍有至少 5 个节点或 4 条关系，或源字幕只是 8 字以内的孤立尾句，也继续只显示一个 supporting cue；只有低密度、完整语义的安静终帧恢复完整句。cue 必须表达动作、因果或结论，不能逐字重复任何当前可见节点标题。当前 16:9 横版的完整句保持 46px，supporting cue 降为 36px；竖版兼容值分别为 42px 和 34px。系统不得截断完整句子伪造短提示，也不得从多节点名称自动生成“Agent等2项”这类占位文案；一个阶段同时聚焦多个对象时，必须显式声明合格 `cueText`，缺少时 fail closed。自动回退若只能得到单个可见节点名称，必须保留 `auto-node` 来源并在展示层同时隐藏阶段说明和底部 cue，不能把同一节点名再显示一遍；隐藏不能改写旁白源字幕。只有作者明确写出的短提示，或完整、未截断且不重复节点名称的短阶段说明，才能进入画面。这样旁白内容保持完整，但画面不会同时要求观众精读大字幕和追踪关系图。
...
- 连接端点必须绑定实际可见承载：完整信息卡绑定四边框，图标节点绑定图标与文字共同占用，纯文字开放节点默认绑定左对齐文字紧边界；不得把用于排版分配的宽空白 cell 冒充可见端点。`package-anatomy` 的居中目录树节点是唯一的层级例外：节点文字必须在分配框内居中，父子线才可以绑定同轴的上下边中心。虚线语义边界和具有全宽可见表面的开放原语仍绑定完整外形；
- 正交路由会把节点列间、行间的真实空隙作为候选通道，避免为了最短距离沿卡片边缘制造双边框。`stable-final` 场景先按最终全部节点确定一次路线，阶段推进只控制显隐；已经可见的关系不得在后续阶段重新寻路、折角跳位或瞬移；
...
- `package-anatomy` 使用注册的 `progressive-package` 层级 profile：核心树只消费 `contains` 关系，`contrasts-with` 等反例进入边界外的独立旁注区，不得参与子节点行居中；只有根节点时也预留下一层的垂直节奏，使首个子项出现时父节点不跳位；
- 渐进目录的 `full-outline` 必须声明 `boundsMode: "visible-members"`，边界只包住当前已出现成员，并在 18 帧内随子树平滑扩展。不得在首个成员出现时一次性画出包含未来成员的完整空框；现有节点的重排与边界扩展使用同一 `stageTransitionProgress`；
- `hierarchy + contains` 使用父节点下边中心、共享竖向主干、水平分支轨和子节点上边中心。单一父子必须退化为一条短竖线；同父多子允许共享主干，不能再由通用最短路由生成互不相关的长 L 线；
- `package-anatomy` 增加目录成员时，必须先完成外框扩展，再把 `contains` 结构线画到新成员，最后才让成员文字进入。成员名属于节点本身，在节点进入前不得以字幕短提示、框外标签或重复文本预显；禁止边框穿字，也禁止节点先悬空、稍后再补关系线；
- `contrasts-with` 必须先完成目录重排，再让边界外的对照对象在最终位置成为可见端点，最后连接对照线；对照线只允许连接完整边界与对照对象边框之间的外部间隙，当前使用紫色短虚线。不得让对照对象在边界内或与边界重叠时预显，不得从目录内部穿过边框，也不得用与 `contains` 相同的实线把它伪装成第五个目录子项；
- 每个阶段可以显式声明 `visibleNodeIds` 与 `visibleEdgeIds`，把同时可见对象限制在当前讲解所需范围。节点的图形、文字和完整边界必须先进入并稳定，关系线才允许从真实 source 绘向真实 target；任何残线、折角或箭头都不得早于两端节点出现。同阶段多条入边或多跳链可以在全部相关端点稳定后同步绘制，但禁止从尚未出现的中间节点提前画线，也禁止箭头指向空白；
...
动效只表达进入、顺序、关系、状态和强调，并由 Remotion frame 驱动。大标题先出现，小字和字幕至少后置 12 帧，主图和细节继续分层进入；禁止 CSS animation/transition。相邻场景的语义内容采用不透明硬交接：边界任一帧只能有一个标题、说明和图解 owner，且有效不透明度必须为 1；标题可在不可见的入场层预滚 11 帧，从边界第一帧起完整显示。交叉淡化只允许用于独立背景、水印或不承载信息的装饰层，不能淡化标题、文字、卡片、图标和箭头。带标签的分组边框必须绑定成员节点的真实入场进度，所有成员仍为 0 时不得提前显示空容器。
