...
- 文字保持第一层级。AI 技术注册图标只允许两种独立职责：`open-diagram-symbol` 用于真正承担对象、动作或交互识别的开放图解对象；`standalone-focus` 用于镜头明确预留空间的单一图标焦点。普通 `card-led` 和结果总结镜头不得临时在卡片旁追加图标；
- 注册图标不得充当卡片徽章、贴边附件、标题前缀或结果卡注释，也不得依附卡片边框、编号或正文定位。开放图解图标可以通过 `anchorId` 建立语义关系，但仍须是独立对象；焦点图标必须使用专门预留的构图区域，不能把四向安全槽冒充焦点布局；
- 对号与数据库、路由、模型、工具等其他注册图标同级，没有覆盖卡片或贴近结果节点的特殊权利。它只能在专门的独立状态图标演示或成功状态焦点布局中出现；
- `open-diagram-symbol` 必须通过安全槽计算：候选位置需完整落在安全区内，并避开全部节点、文字容器和连线；找不到合法槽位时返回 `render: false`。`standalone-focus` 则必须由专用焦点布局预留区域；两者都不得靠缩小、裁切、覆盖或强行挤入来显示；
