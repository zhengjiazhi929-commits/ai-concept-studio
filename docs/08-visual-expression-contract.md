...
- `mixed-diagram` 中至少一个开放图解对象必须声明 `carriesRelation: true`；注册图标可以作为真正承担对象、动作或交互识别的独立开放图解对象，也可以进入专门的焦点布局，但不能成为卡片附件，更不能冒充关系图解；
...
- 语义节点标题不低于 28px，详情文字为 18–24px，阶段标题不低于 46px；低于 90px 的密集信息卡只能使用注册的 30/18px 紧凑档，不能按场景任意缩字；
...
- 测量到标题放不下时，先加宽该卡片；通用自动布局在当前行不足时重排为最多 2 行，明确声明语义行组的长视频镜头最多 3 行；不得为保持固定卡片数量而缩字或断词；
- 三行不是任意堆叠开关，只能通过 `flowLayoutProfile.rowGroups` 声明完整、无重复、无遗漏的语义行，并为每行声明 LTR/RTL 阅读方向；在最大安全宽度和允许行数内仍不能容纳时必须 fail closed，返回修改文案或拆镜头，不让溢出、遮挡或裁切进入渲染；
- 长视频行布局以 `targetRowFillRatio=0.78` 作为可读密度目标，同时受单卡最大宽度约束；它只把确有剩余空间的卡片适度加宽，不靠整体缩放、扩画布或拉伸单个图标填满画面。
- 排版分为共享 `standard` 与重点长片 `longformEmphasis` 两档。长片大字档只能由明确的镜头 profile 启用，不能全局覆盖其他镜头；`visualSystemV1SemanticTextBoxMetrics` 必须按实际 3px 边框、内边距、marker、紧凑档与换行行数检查每个节点。节点矩形不相交但真实文字盒越界仍判定失败。
...
- 承载一致性还必须检查真实关系拓扑：未声明边界的同一连通流程只能使用一种 `surfaceRole`，不能因为节点角色或层级不同，就在主路径上随机把某些节点套卡片、其余节点留在开放画布；
- 合法的承载切换必须位于两个不同 `semanticGroupId` 之间，并在关系上显式声明 `semantic-subgroup-transition`、`surface-change` 与非空 `rationale`，说明从过程、材料或关系切换到完整对象/结果如何增强理解；缺少解释时 fail closed；
- 互不连接的子图、真正独立的结果容器和旁注仍可采用不同承载方式，因此这不是整屏“全卡片”或“全开放”的绝对规则。作为 `graph-node` 的注册图标继承其 anchor 所在连通流程的承载方式，不能借图标绕过一致性检查；
...
- 只要一个对象承载“标题 + 解释/状态”等完整信息，就必须先按信息职责判断是否为信息卡，不能为了省边框把它伪装成开放画布。开放画布只保留真正承担关系端点、过程锚点、状态或注释的对象；因此不会再出现视觉上像卡片却缺少边框的中间形态；
...
- 同一片子复用同一扁平二维语法、颜色 token、线宽、圆角、光学安全边距和注册尺寸角色；不允许场景私有插画风格；
- 开放图解只消费 `grammar-layout.mjs` 注册的通用语义原语，由解析器为当前内容选择组合；这些流程锚点、决策点、输出和关系线是图解结构，不是卡内图标。镜头不得自创 SVG、私有配色或一次性插画组件；
- 文字保持第一层级。AI 技术注册图标只允许三种语义参与方式：`graph-node` 替换并承担真实关系节点，`owned-callout` 在 48px 内归属一个明确 owner，`dedicated-focus` 使用镜头明确预留的单一图标焦点。普通 `card-led` 和结果总结镜头不得临时在卡片旁追加图标；
- 注册图标不得充当卡片徽章、贴边附件、标题前缀或结果卡注释，也不得依附卡片边框、编号或正文定位。`graph-node` 必须成为连接线真实端点，并取代同一语义的文字节点；它的 DOM 渲染框与连线命中框必须共同取自“图标 + 标题 + 说明”的真实内容测量，不能继续使用远大于可见内容的整块布局锚点。`owned-callout` 必须带 owner、归属线和最大 48px 距离；同一 `semanticObjectId` 禁止出现两份主表现；
- 对号与数据库、路由、模型、工具等其他注册图标同级，没有覆盖卡片或贴近结果节点的特殊权利。它只能在专门的独立状态图标演示或成功状态焦点布局中出现；
- 横版长视频禁止使用 `open-diagram-rail`、`left-rail`、`right-rail` 等远端边栏：它会让图标与所解释对象断开，并产生一眼全是图案、文字却难以对应的问题；
- 图标与自己的标题/说明必须在同一阶段揭示，最大时间差 1 帧。`standalone-focus` 必须由专用焦点布局预留区域；任何模式都不得靠缩小、裁切、覆盖或强行挤入来显示；
- 当前长片 S18 不显示对号。该镜头只用完整边框信息卡、正交关系和文字层级表达最终治理结论，不能再把对号作为结果卡注释或结尾徽章加回去。
...
- 长片连接线使用共享 tone：普通关系为 `2.3px / 6.2px marker / 0.54–0.84 opacity`，关键顺序关系可显式使用 `sequence-critical` 的 `2.6px / 7px marker / 0.64–0.90 opacity`；tone 只能增强辨识度，不能改变端点、方向或正交路由；
- 布局默认使用 `stable-final`：先按完整关系图确定最终几何，各阶段只控制显隐，不重排已经出现的节点。只有明确声明 `explicit-reflow` 并提供 `reflowJustification` 时才允许阶段重排；
- 每个阶段可以显式声明 `visibleNodeIds` 与 `visibleEdgeIds`，把同时可见对象限制在当前讲解所需范围。`visibleNodeIds` 先为稳定布局预留端点几何，不等于立即显示节点正文：已有 source 可以在阶段交接期间先绘制关系，新 target 只在最后一条必要入边的箭头抵达时显现；新 source 必须与至少一条出边同帧入场；同阶段多跳链按拓扑波次逐段建立，禁止从尚未建立的中间节点提前画下一条线。没有可确定入口的同阶段循环直接 fail closed；
