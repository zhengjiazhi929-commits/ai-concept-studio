...
function candidatePaths() {
  const fixed = LOCAL_OFFLINE_TTS_V002_REGISTRATION;
...
function assertRegistrationInput(input, verification) {
...
async function inspectCandidateForEpisode(episode, options = {}) {
  if (episode.id !== LOCAL_OFFLINE_TTS_V002_REGISTRATION.episodeId) {
  ...
  const [manifestFile, wavFile] = await Promise.all([
    assertNoSymlinkRegularFile(paths.manifestPath, studioOutputRoot, options),
    assertNoSymlinkRegularFile(paths.wavPath, studioOutputRoot, options)
  ]);
...
function inspectionResponse(verified) {
  const fixed = LOCAL_OFFLINE_TTS_V002_REGISTRATION;
  return {
...
export async function inspectLocalOfflineTtsCandidate(episodeId, options = {}) {
  if (episodeId !== LOCAL_OFFLINE_TTS_V002_REGISTRATION.episodeId) {
  ...
  return inspectionResponse(await inspectCandidateForEpisode(episode, options));
}
...
function assertRegisteredVoiceBinding(episode, verified, expectedPath, expectedPublicPath) {
  const fixed = LOCAL_OFFLINE_TTS_V002_REGISTRATION;
...
async function readJsonArtifact(relativePath, label, options = {}) {
...
async function inspectImmutableRegisteredVoiceSource(episode, options = {}) {
  ...
  assertRegisteredVoiceBinding(episode, verified, expectedPath, expectedPublicPath);
...
export async function inspectRegisteredLocalOfflineTtsRebindCandidate(
  episodeId,
  options = {}
) {
  if (episodeId !== LOCAL_OFFLINE_TTS_V002_REGISTRATION.episodeId) {
  ...
  const priorReviewManifestArtifact = await readJsonArtifact(
    LOCAL_OFFLINE_TTS_REBIND_PRIOR_REVIEW.manifestPath,
    "Storyboard v4 未登记审阅件 manifest",
    options
  );
  assertEqual(
    sha256(priorReviewManifestArtifact.data),
    LOCAL_OFFLINE_TTS_REBIND_PRIOR_REVIEW.manifestSha256,
    "Storyboard v4 未登记审阅件 manifest 哈希不匹配"
  );
  const priorReviewMediaPath = ensureInside(
    workspaceRoot,
    resolve(workspaceRoot, LOCAL_OFFLINE_TTS_REBIND_PRIOR_REVIEW.mediaPath)
  );
  ...
  assertEqual(
    priorReviewMediaData.length,
    LOCAL_OFFLINE_TTS_REBIND_PRIOR_REVIEW.mediaBytes,
    "Storyboard v4 未登记审阅件字节数不匹配"
  );
  assertEqual(
    sha256(priorReviewMediaData),
    LOCAL_OFFLINE_TTS_REBIND_PRIOR_REVIEW.mediaSha256,
  ...
  assertEqual(
    ...
    LOCAL_OFFLINE_TTS_REBIND_PRIOR_REVIEW.storyboardVersion,
  ...
  const priorReviewEvidence = {
    ...
    manifest: {
      path: LOCAL_OFFLINE_TTS_REBIND_PRIOR_REVIEW.manifestPath,
      bytes: priorReviewManifestArtifact.data.length,
      sha256: LOCAL_OFFLINE_TTS_REBIND_PRIOR_REVIEW.manifestSha256
    },
    media: {
      path: LOCAL_OFFLINE_TTS_REBIND_PRIOR_REVIEW.mediaPath,
      bytes: priorReviewMediaData.length,
      sha256: LOCAL_OFFLINE_TTS_REBIND_PRIOR_REVIEW.mediaSha256,
  ...
  const checks = [
    ...
    rebindCheck("prior-v4-review-same-wav", "此前 Storyboard v4 审阅件使用同一 voice-v001", {
      ...
      mediaSha256: LOCAL_OFFLINE_TTS_REBIND_PRIOR_REVIEW.mediaSha256,
      storyboardVersion: 4,
...
export async function verifyRegisteredLocalOfflineVoiceForAssets(episode, options = {}) {
  ...
  if (
    ...
    || voice.sha256 !== LOCAL_OFFLINE_TTS_V002_REGISTRATION.wavSha256
  ) {
    throw registrationError(
...
export async function registerApprovedLocalOfflineTts(episodeId, input, options = {}) {
  if (episodeId !== LOCAL_OFFLINE_TTS_V002_REGISTRATION.episodeId) {
    throw registrationError(
  ...
  try {
    ...
    if (isLocalOfflineRegistration(episode)) {
      const existing = await verifyRegisteredLocalOfflineVoiceForAssets(episode, options);
      assertRegistrationInput(input, existing);
      await recordEvent({
    ...
    assertRegistrationInput(input, verified);
    ...
    assertRegistrationInput(input, currentVerification);
    assertEqual(
