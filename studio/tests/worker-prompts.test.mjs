...
test("Worker Prompt 文件具有稳定 ID、版本和内容哈希", async () => {
  ...
  assert.equal(binding.version, "worker-prompts-v1");
  assert.equal(binding.hash, "ebca66d8de7daeed9fe5b6cde252a3600fa3a39a2a81c5bd14aa1115cd1f3471");
  assert.deepEqual(
    binding.prompts.map(({ workerId, id, version }) => ({ workerId, id, version })),
    [
      { workerId: "asset-agent", id: "acs.worker.asset-plan", version: "1.0.0" },
      { workerId: "script-agent", id: "acs.worker.script-draft", version: "1.0.0" },
      { workerId: "storyboard-agent", id: "acs.worker.storyboard-draft", version: "1.0.0" }
    ]
  );
    ...
    client: {
      async generateStructured(task, request) {
        ...
        return fakeResult({
          ...
          scenes: [
            { type: "title", durationSeconds: 150, kicker: "A", title: "A", statement: "A", subtitle: "A", label: "A", assetHint: "A", subtitleLines: [{ text: "A", weight: 1 }] },
            { type: "evidence", durationSeconds: 150, kicker: "B", title: "B", statement: "B", subtitle: "B", label: "B", assetHint: "B", subtitleLines: [{ text: "B", weight: 1 }] },
            { type: "statement", durationSeconds: 150, kicker: "C", title: "C", statement: "C", subtitle: "C", label: "C", assetHint: "C", subtitleLines: [{ text: "C", weight: 1 }] },
            { type: "summary", durationSeconds: 150, kicker: "D", title: "D", statement: "D", subtitle: "D", label: "D", assetHint: "D", subtitleLines: [{ text: "D", weight: 1 }] }
          ],
          assetChecklist: []
        });
      }
