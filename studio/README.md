...
面向 AI 产品经理的视频 Agent 制作系统。目标是把近期正在形成共识的 AI 技术概念，转化为有证据、可审批、能持续维护的视频；时长由内容和自然语速决定，不为达到十分钟拉慢旁白。长期母版为 16:9，并可重构为 9:16。

当前版本不是自动发布工具。它会完成发现、整理、生成、渲染和检查，但选题仍由人决定，并固定保留研究、脚本、分镜、素材、最终成片五个人工审批闸门。

## 当前进度

代码整合状态与验收边界以 [STATUS](../docs/STATUS.md) 为准；下列 Episode 信息是历史里程碑，实际 Gate 必须读取当前一期状态。
...
- 系统代码具备把结构化数据与已批准素材合成为 Remotion MP4 并执行完整媒体机器 QA 的能力；当前 `golden-001` 的 Research v1、Script v1、Storyboard v1 已批准，Assets/Voice 与 Final Gate 仍未批准，尚无当前有效旁白、MP4、完整 1x 人工观看或最终 QA 结论。
...
- `pnpm illustration:proof --stills-only`：二维过程插画，按需加载样例关键帧；去掉该参数才生成20秒样例动画。
- `pnpm storyboards:proof`：加载、工具调用、路由三组静态分镜，不生成动画。
- `pnpm cards:proof`：微磨砂卡片三种静态布局。
- `pnpm illustration:test`：视觉模块与语义状态专项回归。

以上组件和素材规则可复用，但新内容仍需自己的分镜、时序、旁白及整片检查，不能直接视为成片。
历史长片脚本的十分钟与18场景合同不适合作为任意新视频的默认入口。
完整入口、范围及制作顺序见 [新视频制作基线](../docs/17-new-video-baseline.md)。
